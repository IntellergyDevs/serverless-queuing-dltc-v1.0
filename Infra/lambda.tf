...
# Lambda Function Archive for UpdateTicketHandler
data "archive_file" "lambda_zip_update" {
  type        = "zip"
  source_dir  = "../lambda"
  output_path = "../lambda/UpdateTicketHandler.zip"
}
...
# Lambda Function for Updating Tickets
resource "aws_lambda_function" "UpdateTicketHandler" {
  function_name = "UpdateTicketHandler"
  filename      = data.archive_file.lambda_zip_update.output_path
  handler       = "UpdateTicketHandler.lambda_handler"
  runtime       = "python3.8"
  environment {
    variables = {
      REGION       = "us-east-1"
...
# CloudWatch Log Group for UpdateTicketHandler Lambda Function
resource "aws_cloudwatch_log_group" "lambda_log_group_update" {
  name              = "/aws/lambda/${aws_lambda_function.UpdateTicketHandler.function_name}"
  retention_in_days = 14
}